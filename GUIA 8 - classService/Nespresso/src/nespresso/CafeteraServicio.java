...
public class CafeteraServicio {
    ...
    public CafeteraServicio(int capacidad){
        this.c.setCapacidadMaxima(capacidad);
    }
    ...
    public void servirTaza (int llenarTaza){
        if ( capaTaza >= llenarTaza){
            System.out.println("Su taza ha recibido "+ llenarTaza +" cm3 de café");
            System.out.println("Le falta "+ (capaTaza -= llenarTaza) +" cm3 para ser completada");
            c.setCantidadActual(c.getCantidadActual() - llenarTaza);

        }else{ 
            c.setCantidadActual((c.getCantidadActual() - capaTaza));
            System.out.println("Su taza se ha llenado");
            System.out.println("Y la cafetera quedo con "+ c.getCantidadActual() +" cm3");
        }
    }
